# utils.py

import cv2
import random
from ultralytics import YOLO

# Load your model
model = YOLO('models/yolov8n.pt')  # Replace with your model path

# Define or generate colors for each class
def generate_colors(num_colors):
    return [(random.randint(0, 255), random.randint(0, 255), random.randint(0, 255)) for _ in range(num_colors)]

# Assuming you know the number of classes or can get it from the model
num_classes = len(model.names)  # Or set manually
colors = generate_colors(num_classes)

def display_results(image, results):
    for *xyxy, conf, cls in results:
        label = f'{model.names[int(cls)]} {conf:.2f}'
        plot_one_box(xyxy, image, label=label, color=colors[int(cls)], line_thickness=3)
    cv2.imshow('Results', image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

def plot_one_box(xyxy, img, color=(128, 128, 128), label=None, line_thickness=3):
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(xyxy[0]), int(xyxy[1])), (int(xyxy[2]), int(xyxy[3]))
    cv2.rectangle(img, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
    if label:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(img, c1, c2, color, -1, cv2.LINE_AA)  # filled
        cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
